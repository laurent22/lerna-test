...
exports.mergeWithAdjacentLists = exports.toggleList = void 0;
const BookmarkManager_1 = require("tinymce/core/api/dom/BookmarkManager");
...
const Indendation_1 = require("./Indendation");
const Events_1 = require("../api/Events");
const Util_1 = require("../core/Util");
const ListAction_1 = require("../core/ListAction");
...
const setAttribs = function (elm, attrs) {
    Tools_1.default.each(attrs, function (value, key) {
        elm.setAttribute(key, value);
    });
};
const updateListAttrs = function (dom, el, detail) {
    setAttribs(el, detail['list-attributes']);
    Tools_1.default.each(dom.select('li', el), function (li) {
...
const removeStyles = (dom, element, styles) => {
    Tools_1.default.each(styles, (style) => dom.setStyle(element, { [style]: '' }));
};
...
const getSelectedTextBlocks = function (editor, rng, root) {
    ...
    Tools_1.default.each(siblings, function (node) {
...
const applyList = function (editor, listName, detail = {}) {
    ...
    Tools_1.default.each(getSelectedTextBlocks(editor, rng, root), function (block) {
...
const updateList = function (editor, list, listName, detail) {
    if (list.nodeName !== listName) {
        ...
        Events_1.fireListEvent(editor, ListAction_1.listToggleActionFromListName(listName), newList);
    }
    else {
        updateListWithDetails(editor.dom, list, detail);
        Events_1.fireListEvent(editor, ListAction_1.listToggleActionFromListName(listName), list);
    }
};
const toggleMultipleLists = function (editor, parentList, lists, listName, detail) {
    if (parentList.nodeName === listName && !hasListStyleDetail(detail)) {
        ...
        Tools_1.default.each([parentList].concat(lists), function (elm) {
...
const toggleSingleList = function (editor, parentList, listName, detail) {
    ...
    if (parentList) {
        const listType = JoplinListUtil_1.findContainerListTypeFromElement(parentList);
        if (parentList.nodeName === listName && !hasListStyleDetail(detail) && !Util_1.isCustomList(parentList) && listType === detail.listType) {
            Indendation_1.flattenListSelection(editor);
        }
        else {
            const bookmark = Bookmark.createBookmark(editor.selection.getRng(true));
        ...
        Events_1.fireListEvent(editor, ListAction_1.listToggleActionFromListName(listName), parentList);
    }
};
const toggleList = function (editor, listName, detail) {
    ...
    detail = Object.assign({ listType: 'regular' }, detail);
