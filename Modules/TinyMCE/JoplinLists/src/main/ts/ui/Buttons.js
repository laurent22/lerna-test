...
const listState = function (editor, listName, options = {}) {
    ...
    return function (buttonApi) {
        const nodeChangeHandler = (e) => {
            ...
            const listType = JoplinListUtil_1.findContainerListTypeFromEvent(e);
            buttonApi.setActive(listType === options.listType && lists.length > 0 && lists[0].nodeName === listName && !Util_1.isCustomList(lists[0]));
        };
        const editorClickHandler = (event) => {
            if (!JoplinListUtil_1.isJoplinChecklistItem(event.target))
                return;
