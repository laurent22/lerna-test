...
const outdentedComposer = (editor, entries) => {
    return katamari_1.Arr.map(entries, (entry) => {
        ...
        return sugar_1.Element.fromDom(TextBlock_1.createTextBlock(editor, content.dom()));
    });
};
...
const indentSelectedEntries = (entries, indentation) => {
    katamari_1.Arr.each(katamari_1.Arr.filter(entries, Entry_1.isSelected), (entry) => Indentation_1.indentEntry(indentation, entry));
};
